...
class TestPluginInjector extends SyntheticMembersInjector {
  override def injectFunctions(source: ScTypeDefinition): Seq[String] = {
    source match {
      case c: ScClass if (c.hasAnnotation("grid.Gridzzly").isDefined || c.hasAnnotation("grid.GridAnnotation").isDefined) && c.extendsBlock.superTypes.headOption.isDefined =>
        ...
        Seq(s"def run(conditions: GridConditions)(implicit dbConnection: DBConnection): (_root_.scala.concurrent.Future[Seq[$secondType]], _root_.scala.concurrent.Future[Int]) = ???",
